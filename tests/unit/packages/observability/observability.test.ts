...
describe("structured observability", () => {
  it("keeps correlation and duration fields while removing secrets and complete env maps", () => {
    ...
    const record = logger.log({
      ...
      stage: "worker",
      durationMs: 12,
      correlation: { projectId: "project-1", jobId: "job-1", revision: 2, attemptId: "attempt-1" },
      data: {
        token: "token-value",
    ...
    expect(record.correlation).toMatchObject({ jobId: "job-1", revision: 2, attemptId: "attempt-1" });
