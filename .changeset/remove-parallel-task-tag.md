...
Removing the field also changes `tasksHash` and the manifest hash for identical task sources. `sourceHash` is unaffected because the Daemon still digests the raw task bytes when it creates the immutable Job artifacts.
