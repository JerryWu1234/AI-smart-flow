...
Upgrade the local IPC contract to `smartflow.v6`. `ReviewIssue.suggestedFix` is now required and nullable (`string | null`), with the Review JSON Schema generated directly from the shared protocol schema.