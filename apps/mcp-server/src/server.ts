  ...
  const server = new McpServer(
    { name: "smartflow", version: "0.1.0" },
    {
      instructions: [
        ...
        "On USER_INPUT_REQUIRED, present pause.message to the user and submit exactly one options[].answer with the turnToken. result.review carries the latest recorded per-Task completion and issues when a Review exists. requiredInput may be absent; for COLLECT, collect every field in inputForm and submit the complete structured answer only after user approval, and for CONFIRM, ask the user to confirm requiredInput.answer unchanged before submitting it. result carries the paused run snapshot, including artifacts for the durable evidence behind the pause.",
