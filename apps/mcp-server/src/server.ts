...
export function createSmartFlowMcpInstructions(tasksPath: string): string {
  return [
    ...
    "Generate canonical SmartFlow syntax: use ## M01 module headings; - [ ] T001 tasks with unique IDs; only [M01]-style module labels; at least one backtick-wrapped target path per task; the exact separator ' — Acceptance:' before acceptance criteria; and at least one incomplete task.",
