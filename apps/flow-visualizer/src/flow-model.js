...
const STAGE_LIST = [
  ...
  stage({
    ...
    summary: "把当前结果冻结为从 Run baseline 到最终结果的累计差异。",
    plainLanguage: "Reviewer 和发布都检查同一份完整候选，不会只看到某一轮小补丁，也不会检查还在变化的目录。",
    before: { phase: "RUNNING", resultSnapshot: "mutable workspace", candidate: "—" },
    after: { phase: "RUNNING", resultSnapshot: "frozen", candidate: "hash-bound" },
    outputs: ["Candidate", "result snapshot", "incremental + cumulative patch"],
  ...
  stage({
    ...
    actorIds: ["daemon", "ledger", "workspace", "project"],
    summary: "生成稳定 operationId，对所有目标路径执行 expected-old hash/mode 检查。",
    plainLanguage: "先一次性检查所有文件，再允许第一笔写入。任一路径被用户改过都会以零写入暂停。",
  ...
  stage({
    ...
    plainLanguage: "只有原项目最终内容和权限都符合预期，Run 才会标记完成；临时目录可清理，但证据继续保留。",
    before: { phase: "PUBLISHING", publishResult: "pending" },
    after: { phase: "COMPLETED", publishResult: "COMMITTED", projectWrite: "APPLIED" },
    outputs: ["terminal RunRecord", "committed PublishResult", "retained artifacts"],
...
const MAIN = [
  ...
  transition({
    ...
    explanation: "生成签名 DeliveryBundle、expected-old CAS operations 和稳定 operationId。",
