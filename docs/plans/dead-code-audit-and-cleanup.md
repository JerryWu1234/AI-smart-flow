...
>
> **2026-09-05 补充清理：** SQLite 物理 schema 现为 6，删除无人消费的 `mutation_lease.owner_id`、回执中的重复 `requestId` 和未使用的 `committedAtStateVersion`；停止写入 `recovery.phase/action`，移除未被 Worker 启动使用的 `recoveryEpoch`。项目尚未发布，继续采用 latest-only：只支持当前布局，不迁移、不兼容读取旧数据；本地开发数据可用 `pnpm clean:daemon-data` 清理后重建。
>
