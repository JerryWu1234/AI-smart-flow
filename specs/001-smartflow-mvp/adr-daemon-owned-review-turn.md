...
3. The Daemon internally owns deterministic waiting, Review Action claim and renewal, Review result submission, automatic accept/repair/pause planning, approved-scope repair continuation, and Publish progression.
...
7. Per-Run serialization, Project-wide CAS, stable internal-operation request IDs, restart recovery, deadline enforcement, and claim renewal are Daemon responsibilities.
...
- The fifteenth incomplete repair Review pauses as `AUTOMATIC_REPAIR_LIMIT`; the owning Host may grant another group by submitting `resume_review_decision` through `smartflow_review_turn` with the active `turnToken`. HostTurnCoordinator then invokes Daemon resume mechanics internally and resets the counter.
...
- The Daemon maintains per-Run queues and renewal/deadline timers and owns the internal wait, Action claim/renew, Review submission, decision, repair, and Publish operations.
...
- **Return worktree paths from status/pause**: rejected because path authority must follow a current successful internal claim.
