...
- [ ] T155 [US1] Replace installed OpenCode probing with Pi SDK/runtime/in-memory-model probing in `apps/cli/src/doctor.ts`, `apps/cli/src/main.ts`, and `apps/cli/src/doctor.test.ts` — Acceptance: CLI reports Pi/Node/Sandbox/model-registration readiness, redacts the direct API Key, and has no Provider-selection or Broker permission-policy checks.
