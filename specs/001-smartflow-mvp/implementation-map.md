...
|---|---|---|---|---|
| FR-042 — sole public `smartflow_execute → smartflow_review_turn*` Review orchestration path, four public states, bounded poll | T201, T205 | `packages/protocol/src/schema/mcp-tools.ts`; `apps/mcp-server/src/server.ts`; `apps/mcp-server/src/tools/review-turn.ts`; `apps/daemon/src/host-turn-coordinator.ts` | `tests/unit/packages/protocol/schema/protocol.test.ts`; `tests/fixtures/installed-mcp-lifecycle-child.mjs`; coordinator and repository Host-simulation tests | Implemented — unanswered pauses return the embedded canonical result without calling an independent Run management API |
| FR-043 — Daemon owns mechanics; Host owns Reviewer/user interaction | T202, T205 | `apps/mcp-server/src/server.ts`; `apps/daemon/src/host-turn-coordinator.ts` | policy/coordinator tests; `tests/helpers/host-workflow/workflow.ts`; `tests/unit/helpers/host-workflow/workflow.test.ts` | Implemented — native MCP Host executes only Reviewer callbacks and user-answer callbacks; the repository helper is test-only |
| FR-044 — Reviewer CREATE/RESUME, durable Host ownership, atomic begin-before-path | T204, T205 | `apps/mcp-server/src/server.ts`; `apps/daemon/src/host-turn-coordinator.ts`; `apps/daemon/src/review-coordinator.ts` | `tests/helpers/host-workflow/reviewer.ts`; `tests/unit/helpers/host-workflow/workflow.test.ts`; direct installed-MCP fixture; protocol path guards; internal-owner integration regression | Implemented — one CAS mutation persists `REVIEWING + AWAITING_REVIEW`; cumulative `changedPaths` and owner/token bindings gate Review continuation |
| FR-045 — schema-v5 `hostTurn`, v4 migration, and `autoRepairRounds` | T203 | `packages/state-store/src/schema.ts`; `packages/state-store/src/state-store.ts`; `apps/daemon/src/project-runtime.ts` | `tests/unit/packages/state-store/schema.test.ts` | Implemented — v4 claim fields are removed during startup migration and ambiguous active Review state safely pauses |
...
| FR-048 — deterministic accept/repair/invalid/15-round policy | T202, T206 | `packages/review/src/review-decision.ts`; `apps/daemon/src/host-turn-coordinator.ts`; `apps/daemon/src/project-runtime.ts` | `tests/unit/packages/review/review-decision.test.ts`; coordinator and production repair-loop tests | Implemented |
| FR-049 — typed `USER_INPUT_REQUIRED`; terminal-only `DONE` | T201, T204, T205 | review-turn schemas; `HostTurnCoordinator.requireUserInput/userInputRequired`; native Host instructions in `apps/mcp-server/src/server.ts` | protocol/coordinator tests and repository-only Host simulation | Implemented — canonical result, mutable options, inspections, `COLLECT` forms, and complete `CONFIRM` answers are disjoint |
| FR-050 — exactly six public MCP tools; one Review path plus four independent Run management APIs | T201 | `apps/mcp-server/src/tools/index.ts`; `packages/protocol/src/schema/mcp-tools.ts`; `apps/mcp-server/src/server.ts` | `tests/contract/mcp-v5.test.ts`; `tests/unit/packages/protocol/schema/protocol.test.ts` | Implemented — the five named public Review-mechanics symbols, schemas, handlers, registrations, and aliases do not exist; those mechanics are Daemon-internal only |
...
| SC-018 — exact six-tool public surface and sole two-tool Review orchestration path | T201, T205; protocol/MCP contract, direct installed-MCP fixture, and repository Host-simulation tests | Met locally — the registry exposes only `smartflow_execute`, `smartflow_review_turn`, and four independent Run management APIs |
